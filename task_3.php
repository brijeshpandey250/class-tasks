<?php
	/*
		* method to use new line character \n as number add example: 2\n3,4
	...
	*/
	...
	class AddClass{
		...
		function add($argv){
			...
			*/
					...
					if(isset($argv[1])){
						...
						if($argv[1] == 'add'){
							...
							if(isset($argv[2])){
								...
								$value = $argv[2];
								// set the pattern to accept new line character
								$pattern = '/[n,]/';
								// convert third argument from string to an array
								$tempArray = preg_split( $pattern, $value );
								/*
									* call method checkAllValues	
									* to check all vaues in array should be positive number
								*/
								$checkResult = $this->checkAllValues($tempArray);
								if($checkResult == 1){
									echo "\n"."Write  command as: task_3.php add 2\n3,4"."\n\n";
								} else {
			...
			if($errorFlag == 1){
				echo "\n"."Please supply required arguments "."\n";
				echo "\n"."Write  command as: task_3.php add 2\n3,4"."\n\n";
			}
		}
	...
	// create object of class AddClass
	$obj = new AddClass();
